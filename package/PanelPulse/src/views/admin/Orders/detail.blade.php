        ...
        <div class="container mb-4">
            <div class="row">
                ...
                <div class="col-md-6 text-right">
                    <p class="mb-0"><button class="btn btn-link"
                            onclick="window.open('/admin/orders/{{ $orders['id'] }}/invoice')">Print Invoice</button></p>
                </div>
            </div>
            <div class="row">
                <div class="col-md-1"></div>
                <div class="col-md-11 pl-0">
                    <p class="subtext1 mb-0">@php
                        echo date_format($orders['created_at'], 'd F Y');
                        echo ' at ';
                        echo date_format($orders['created_at'], 'h:i a');
                    @endphp</p>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                        ...
                        @foreach ($orders_items as $orders_item)
                            <div class="row m-0 mb-4">
                                <div class="col-6 pro-flex p-0">
                                    <div class="img-block"
                                        style="background-image:url('/storage/shop/@php echo str_replace(' ','-',$orders_item['product_modelNo']) @endphp/{{ $orders_item['product_pic1'] }}');">
                                    ...
                                    <div class="text-block">
                                        <table style="width:100%;height:100%">
                                            <tr>
                                                <td class="align-middle" style="width:100%;height:100%">
                                                    <p class="mb-0"><a
                                                            href="/shop/@php echo str_replace(' ','-',strtolower($orders_item['product_category'])) @endphp/{{ $orders_item['product_url'] }}"
                                                            target="_blank">{{ $orders_item['product_name'] }}</a></p>
                                                    <p class="mb-0">SKU: @if ($orders_item['product_isVariant'] == 'No')
                                                            {{ $orders_item['product_modelNo'] }}
                                                        @else
                                                            {{ $orders_item['product_variationSKU'] }}
                                                        @endif
                                                    </p>
                                                    @if ($orders_item['product_variationValue'] != '')
                                                        <p class="mb-0">{{ $orders_item['product_variationType'] }}:
                                                            {{ $orders_item['product_variationValue'] }}</p>
                                                    @endif
                                                </td>
                                            </tr>
                                        </table>
                            ...
                            <div class="row m-0 pt-3" style="border-top:1px solid #dddddd;">
                                ...
                                <div class="col-12">
                                    ...
                                    <p><a href="{{ $orders['shipping_tracking_link'] }}"
                                            target="_blank">{{ $orders['shipping_tracking_link'] }}</a></p>
                                </div>
                            </div>
                        @endif
                        <div class="text-right pt-3 mb-0" style="border-top:1px solid #dddddd;">
                            <div class="dropdown mr-3" style="display:inline-block;">
                                ...
                                <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton">
                                    <a class="dropdown-item @if ($orders['order_status'] != 'Reviewed') disabled @endif"
                ...
                <div class="col-md-4">
                    <div class="container info-cont">
                        ...
                        <p class="mb-0">{{ $orders_customers['shipping_city'] }},
                            {{ $orders_customers['shipping_state'] }} - {{ $orders_customers['shipping_pincode'] }}</p>
                        ...
                        @else
                            <p class="mb-0">{{ $orders_customers['billing_first_name'] }}
                                {{ $orders_customers['billing_last_name'] }}</p>
                            <p class="mb-0">{{ $orders_customers['billing_address'] }}</p>
                            <p class="mb-0">{{ $orders_customers['billing_city'] }},
                                {{ $orders_customers['billing_state'] }} - {{ $orders_customers['billing_pincode'] }}</p>
                        ...
                        <div class="form-row">
                            <div class="col-md-6 mb-3">
                                <input type="text" class="form-control" id="validationCustom01" name="firstname"
                                    placeholder="First Name" value="{{ $orders_customers['first_name'] }}" required />
                            </div>
                            <div class="col-md-6 mb-3">
                                <input type="text" class="form-control" id="validationCustom02" name="lastname"
                                    placeholder="Last Name" value="{{ $orders_customers['last_name'] }}" required />
                            </div>
                        </div>
                        <div class="form-row">
                            <div class="col-md-12 mb-3">
                                <input type="text" class="form-control" id="validationCustom03" name="address"
                                    placeholder="Address" value="{{ $orders_customers['shipping_address'] }}" required />
                            </div>
                            <div class="col-md-12 mb-3">
                                <input type="text" class="form-control" id="validationCustom04" name="city"
                                    placeholder="City" value="{{ $orders_customers['shipping_city'] }}" required />
                            </div>
                                ...
                                <select class="custom-select" id="validationCustom06" name="state" required>
                                    ...
                                    @foreach ($shippings->unique('state') as $shipping)
                                        <option value="{{ $shipping->state }}"
                                            @if ($orders_customers['shipping_state'] == $shipping->state) selected @endif>{{ $shipping->state }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-4 mb-3">
                                <input type="number" class="form-control" id="validationCustom07" name="pincode"
                                    placeholder="Pincode" value="{{ $orders_customers['shipping_pincode'] }}" required />
                            </div>
                        </div>
                        <div class="form-row">
                            <div class="col-md-12 mb-3">
                                <input type="number" class="form-control" id="validationCustom08" name="phone"
                                    placeholder="Phone" value="{{ $orders_customers['phone'] }}" required />
                            </div>
                        </div>
                        <p class="text-right"><button type="submit" class="btn btn-primary">Save</button></p>
                    </form>
                ...
                <div class="modal-body">
                    <form class="needs-validation" action="/admin/orders/{{ $orders->id }}/update" method="POST"
                        ...
                        <div class="form-row">
                            <div class="col-md-6 mb-3">
                                <input type="text" class="form-control" id="validationCustom01" name="firstname"
                                    placeholder="First Name" value="{{ $orders_customers['billing_first_name'] }}"
                                    required />
                            </div>
                            <div class="col-md-6 mb-3">
                                <input type="text" class="form-control" id="validationCustom02" name="lastname"
                                    placeholder="Last Name" value="{{ $orders_customers['billing_last_name'] }}"
                                    required />
                            </div>
                        </div>
                        <div class="form-row">
                            <div class="col-md-12 mb-3">
                                <input type="text" class="form-control" id="validationCustom03" name="address"
                                    placeholder="Address" value="{{ $orders_customers['billing_address'] }}" required />
                            </div>
                            <div class="col-md-12 mb-3">
                                <input type="text" class="form-control" id="validationCustom04" name="city"
                                    placeholder="City" value="{{ $orders_customers['billing_city'] }}" required />
                            </div>
                            <div class="col-md-4 mb-3">
                                <select class="custom-select" id="validationCustom05" name="country" required>
                                    <option disabled value="">Country/Region</option>
                            ...
                            <div class="col-md-4 mb-3">
                                <select class="custom-select" id="validationCustom06" name="state" required>
                                    ...
                                    @foreach ($shippings->unique('state') as $shipping)
                                        <option value="{{ $shipping->state }}"
                                            @if ($orders_customers['billing_state'] == $shipping->state) selected @endif>{{ $shipping->state }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-4 mb-3">
                                <input type="number" class="form-control" id="validationCustom07" name="pincode"
                                    placeholder="Pincode" value="{{ $orders_customers['billing_pincode'] }}" required />
                            </div>
                        </div>
                        <div class="form-row">
                            <div class="col-md-12 mb-3">
                                <input type="number" class="form-control" id="validationCustom08" name="phone"
                                    placeholder="Phone" value="{{ $orders_customers['billing_phone'] }}" required />
                            </div>
                        </div>
                        <p class="text-right"><button type="submit" class="btn btn-primary">Save</button></p>
                    </form>
